...
namespace MissionPlanner.Log
{
    public partial class LogDownloadMavLink : Form
    {
        ...
        string GetPos(ushort no, string fileName)
        {
            log.Info("GetPos " + no);
            ...
            // get df log from mav
            using (var ms = MainV2.comPort.GetPos(no))
            {
                if (ms != null)
        ...
        private void DownloadThread(int[] selectedLogs,bool pos)
        {
            ...
            try
            {
                ...
                foreach (int a in selectedLogs)
                {
                    ...
                    string s_logname;
                    if (data_type==0)
                        s_logname = GetLog(entry.id, fileName);
                    else
                        s_logname = GetPos(entry.id, fileName);
                    var logname = s_logname;
                    CreateLog(logname);
